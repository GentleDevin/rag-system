...
class TextVector():
    ...
    def get_vec_batch(self, data, bs=None):
        """batch方式获取，提高效率
        把文本变成数字向量
        输入：["你好", "今天天气不错", "AI 检索系统"]
        输出：tensor([[0.123, 0.456, ...], [0.789, ...], ...])
        """
        if bs is None:
            bs = self.batch_size
    ...
    def vector_similarity(self, vectors):
        """
         向量组，以[query，text1，text2...]来计算query与text1，text2,...的cosine相似度，分数越高 = 语义越接近
        输入：[问题向量, 文本1向量, 文本2向量]输出：[0.92, 0.35]表示：
            问题与文本 1 相似度 92%
            问题与文本 2 相似度 35%
        """
...
"""
通用文本向量化 + 相似度计算工具
这是一个文本转向量（Embedding）工具，能把文字 → 数字向量，并且计算两段文字的相似度。
两大核心能力：
文本转向量（本地 BERT 模型 / 阿里云通义千问 API text-embedding-v3）
计算文本相似度（余弦相似度）
作用：给 RAG、检索、匹配、推荐、问答系统 提供最底层的向量支持。
"""