...
def search_bing(query):
    ...
    功能：
    自动搜索关键词 → 抓取搜索结果 → 提取网页正文 → 返回结构化数据
    作用：
    给 AI 提供外部实时知识（解决大模型不知道最新消息的问题）
    input：query

    返回格式：
    [
        {"url":"网址", "title":"标题", "text":"网页正文"},
        ...
    ]
    """
    headers = {
...
"""
联网搜索增强型RAG核心引擎
作用：你输入一个问题 → 自动去 Bing 搜索 → 爬取网页 → 提取关键内容 → 返回最相关的参考文本
→ 给大模型（通义千问 / GPT）提供最新、最准确、最真实的外部知识！"""
cfg = Config()
