    ...
    def process_and_update_chat(question, kb_name, use_search, use_table_format, multi_hop, chat_history):
        ...
        try:
            ...
            for search_display, answer in process_question_with_reasoning(
                    question, kb_name, use_search, use_table_format, multi_hop, chat_history[:-2]
            ):
                last_search_display = search_display
                last_answer = answer

                # ✅ 关键：修改最后一条 assistant 消息的 content，而不是重新 append
                chat_history[-1]["content"] = answer
                yield chat_history, update_status(True), search_display

            # ✅ 3. 处理完成，最终更新
            chat_history[-1]["content"] = last_answer
